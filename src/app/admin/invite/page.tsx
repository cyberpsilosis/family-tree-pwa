...
import { X, Plus, Mail, CheckCircle } from 'lucide-react'
...
interface InviteSuccess {
  ...
  emailSent: boolean
  password?: string // Only if email failed
  error?: string // Only if email failed
}
...
export default function InvitePage() {
      ...
      <div className="relative mx-auto max-w-2xl space-y-6">
        <div className="animate-fade-up">
          <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mb-2">
            {success.emailSent ? 'Success' : 'Partial Success'}
          </p>
          <h1 className="text-4xl font-serif font-light tracking-tight text-foreground md:text-5xl">
            Invite <span className="font-semibold">{success.emailSent ? 'Sent' : 'Created'}</span>
          </h1>
          <p className="mt-3 text-lg text-muted-foreground">
            {success.emailSent 
              ? 'The invite email has been sent successfully'
              : 'Member created but email delivery failed'}
          </p>
        </div>
          ...
          <CardContent className="p-6 space-y-6">
            {success.emailSent ? (
              <>
                ...
                <div>
                  <p className="text-sm text-muted-foreground mb-2">Invited Member</p>
              ...
              <>
                ...
                <div>
                  <p className="text-sm text-muted-foreground mb-2">Member Created</p>
                  <p className="text-lg font-medium">{success.user.firstName} {success.user.lastName}</p>
                  <p className="text-sm text-muted-foreground">{success.user.email}</p>
                </div>
              </>
            )}
  ...
  return (
    <div className="relative mx-auto max-w-2xl space-y-6">
      <div className="animate-fade-up">
        <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground mb-2">Invite</p>
        <h1 className="text-4xl font-serif font-light tracking-tight text-foreground md:text-5xl">
          Send <span className="font-semibold">Invite</span>
        </h1>
        <p className="mt-3 text-lg text-muted-foreground">Invite a new member via email</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  The invite email with login credentials will be sent to this address
                </p>
              </div>
            ...
            <div className="flex gap-4 pt-6 border-t border-border/50">
              ...
              <Button
                ...
                className="flex-1"
              >
                {isLoading ? 'Sending Invite...' : 'Send Invite Email'}
              </Button>
            </div>
          </form>
        </CardContent>
